...
> = {
  "JR-East.Yamanote": { name: "山手線", code: "JY", color: "#9ACD32" },
  ...
  "JR-East.Tokaido": { name: "東海道線", code: "JT", color: "#F68B1E" },
  "JR-East.Yokosuka": { name: "横須賀線", code: "JO", color: "#0067C0" },
  "JR-East.Utsunomiya": { name: "宇都宮線", code: "JU", color: "#F68B1E" },
  "JR-East.Takasaki": { name: "高崎線", code: "JU", color: "#F68B1E" },
  "JR-East.Joban": { name: "常磐線", code: "JJ", color: "#00A7A0" },
  "JR-East.Keiyo": { name: "京葉線", code: "JE", color: "#C9242F" },
  ...
  "Toei.Asakusa": { name: "都営浅草線", code: "A", color: "#E85298" },
  "Toei.Mita": { name: "都営三田線", code: "I", color: "#0079C2" },
  "Toei.Shinjuku": { name: "都営新宿線", code: "S", color: "#6CBB5A" },
  "Toei.Oedo": { name: "都営大江戸線", code: "E", color: "#B6007A" },

  "Keio.Keio": { name: "京王線", code: "KO", color: "#DD0077" },
  "Keio.KeioNew": { name: "京王新線", code: "KO", color: "#DD0077" },
  "Keio.Inokashira": { name: "京王井の頭線", code: "IN", color: "#000088" },

  "Odakyu.Odawara": { name: "小田急小田原線", code: "OH", color: "#2288CC" },

  "Seibu.Shinjuku": { name: "西武新宿線", code: "SS", color: "#00A6BF" },
  "Seibu.Ikebukuro": { name: "西武池袋線", code: "SI", color: "#F58220" },

  "Tobu.Tojo": { name: "東武東上線", code: "TJ", color: "#004098" },

  "Tokyu.Toyoko": { name: "東急東横線", code: "TY", color: "#DA0442" },
  "Tokyu.DenEnToshi": {
    name: "東急田園都市線",
    code: "DT",
    color: "#20A288",
  },
  "Tokyu.Meguro": { name: "東急目黒線", code: "MG", color: "#009CD2" },
  "Tokyu.Ikegami": { name: "東急池上線", code: "IK", color: "#EE86A7" },

  "Keikyu.Main": { name: "京急本線", code: "KK", color: "#00A7E3" },
...
const SUPPORTED_RAILWAYS: Record<string, string> = {
  Yamanote: "JR-East.Yamanote",
  ChuoRapid: "JR-East.ChuoRapid",
  ChuoSobuLocal: "JR-East.ChuoSobuLocal",
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    const railwayId = SUPPORTED_RAILWAYS[railway];

    if (!railwayId) {
      return NextResponse.json(
        {
          error: "지원하지 않는 노선입니다.",
          supportedRailways: Object.keys(SUPPORTED_RAILWAYS),
        },
        { status: 400 },
      );
    }
    ...
    const currentRailway = `odpt.Railway:${railwayId}`;

    const connectingRailways: ConnectingRailway[] = (
      odptStation["odpt:connectingRailway"] ?? []
    )
      .filter((connectingRailway) => connectingRailway !== currentRailway)
